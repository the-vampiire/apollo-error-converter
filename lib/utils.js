 ...
 * @typedef {{ message: string, errorConstructor: function, logger?: boolean | function, data?: {} | function, code: string }} MapItem
 */
...
/**
 * Extracts the arguments required by the MapItems constructor
 * - 1 arg: message
 * - 2 arg: message, data
 * - 3 arg: message, code, data
 *
 * Defaults
 * - data = {}
 ...
 */
function parseConstructorArguments(error, mapItem) {
  const {
    message,
    data = {},
    errorConstructor,
  ...
  const numberOfArguments = errorConstructor.length;
  const errorData = typeof data === 'function' ? data(error) : data;

  return numberOfArguments === 3
    ? [message, code, errorData]
    : [message, errorData].slice(0, numberOfArguments);
}
